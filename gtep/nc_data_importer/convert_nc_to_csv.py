...
def bus_df(bus_data, load_data):
    ...
    bus_dict = {
        "Bus ID": list(bus_data["i"]),
        "Bus Name": list(bus_data["i"]),
        "BaseKV": list(bus_data["v_nom"]),
        "Bus Type": list(bus_data["control"]),
        "MW Load": [np.nan] * num_bus,
        "MVAR Load": [np.nan] * num_bus,
        "V Mag": [np.nan] * num_bus,
        "V Angle": [np.nan] * num_bus,
        "Area": list(bus_data["location"]),
        "Zone": list(bus_data["country"]),
        # extra columns
        "Carrier": list(bus_data["carrier"]),
        "x": list(bus_data["x"]),
        "y": list(bus_data["y"]),
        "sub_network": list(bus_data["sub_network"]),
        "substation_lv": list(bus_data["substation_lv"]),
        "substation_off": list(bus_data["substation_off"]),
    }
...
def branch_df(links, lines):
    ...
    branch_dict = {
        "UID": list(lines["i"]),
        "From Bus": list(lines["bus0"]),
        "To Bus": list(lines["bus1"]),
        "R": [np.nan] * num_branches,
        "X": [0.00287] * num_branches,
        "B": [np.nan] * num_branches,
        "Cont Rating": list(lines["s_nom"]),
        "LTE Rating": list(lines["s_nom"]),
        "STE Rating": list(lines["s_nom"]),
        "Tr Ratio": [1.0] * num_branches,  # if 0.0 this turns it into a tranformer
        # extra columns
        "Carrier": list(lines["carrier"]),
        "s_max_pu": list(lines["s_max_pu"]),
        "capital_cost": list(lines["capital_cost"]),
        "length": list(lines["length"]),
        "num_parallel": list(lines["num_parallel"]),
        "sub_network": list(lines["sub_network"]),
        "v_nom": list(lines["v_nom"]),
        "i_nom": list(lines["i_nom"]),
    ...
    for ix, type in enumerate(links["carrier"]):
        ...
        if type == "DC":
            ...
            dc_dict["under_construction"].append(bool(links["under_construction"][ix]))
...
def gen_df(gens, carriers):
    ...
    for ix, name in enumerate(list(carriers["i"])):
    ...
    gen_dict = {
        ...
        "Start Heat Hot MBTU": [np.nan]
        * num_gens,  # TODO read function default startup fuel to 0
    ...
    for ix, name in enumerate(gens["i"]):
        ...
        if bool(gens["p_nom_extendable"][ix]):
            num_gens += 1  # add this extra one to the count
            gen_dict["GEN UID"].append(f"{name}-c")
            gen_dict["Bus ID"].append(gens["bus"][ix])
            gen_dict["Unit Type"].append(
                CARRIER_ASSIGN[gens["carrier"][ix]]["unit_type"]
            )
            gen_dict["Fuel"].append(CARRIER_ASSIGN[gens["carrier"][ix]]["fuel"])
            gen_dict["PMax MW"].append(gens["p_nom"][ix])
            # extra
    ...
    gen_dict["MVAR Inj"] = [np.nan] * num_gens
    gen_dict["V Setpoint p.u."] = [np.nan] * num_gens
    gen_dict["PMin MW"] = [0] * num_gens
    gen_dict["QMax MVAR"] = [np.nan] * num_gens
    gen_dict["QMin MVAR"] = [np.nan] * num_gens
    gen_dict["Min Down Time Hr"] = [np.nan] * num_gens
    gen_dict["Min Up Time Hr"] = [np.nan] * num_gens
    gen_dict["Ramp Rate MW/Min"] = [1] * num_gens  # TODO read function pull in 1
    gen_dict["Start Time Cold Hr"] = [np.nan] * num_gens
    gen_dict["Start Time Warm Hr"] = [np.nan] * num_gens
    gen_dict["Start Time Hot Hr"] = [np.nan] * num_gens
    gen_dict["Start Heat Cold MBTU"] = [np.nan] * num_gens
    gen_dict["Start Heat Warm MBTU"] = [np.nan] * num_gens
    gen_dict["Start Heat Hot MBTU"] = [
        np.nan
    ] * num_gens  # TODO read function default startup fuel to 0
    gen_dict["Non Fuel Start Cost $"] = [0] * num_gens
    gen_dict["Fuel Price $/MMBTU"] = [1] * num_gens
    # These are used for p_fuel but we have those values saved individually
    gen_dict["Output_pct_0"] = [np.nan] * num_gens
    gen_dict["Output_pct_1"] = [np.nan] * num_gens
    gen_dict["Output_pct_2"] = [np.nan] * num_gens
    gen_dict["Output_pct_3"] = [np.nan] * num_gens
    gen_dict["Output_pct_4"] = [np.nan] * num_gens
    gen_dict["HR_avg_0"] = [np.nan] * num_gens
    gen_dict["HR_incr_1"] = [np.nan] * num_gens
    gen_dict["HR_incr_3"] = [np.nan] * num_gens
    gen_dict["HR_incr_4"] = [np.nan] * num_gens
...
def storage_df(storage, num_hours):
    ...
    stor_dict = {
        ...
        "max_discharge_rate": list(storage["units_p_nom"]),
        "min_discharge_rate": [0] * num_stores,
        "max_charge_rate": list(storage["units_p_nom"]),
...
if __name__ == "__main__":
    ...
    data = {}
    data["bus.csv"], data["bus_load.csv"], data["p_load.csv"], data["q_load.csv"] = (
        bus_df(groups["buses"], groups["loads"])
    )
    data["branch.csv"], data["dc_branch.csv"] = branch_df(
        groups["links"], groups["lines"]
    )
    data["gen.csv"], data["p_fuel.csv"], data["p_cost.csv"] = gen_df(
        groups["generators"], groups["carriers"]
    )
    data["storage.csv"] = storage_df(
        groups["storage"], max(groups["snapshots"]["snapshot"])
    )
    start_time, end_time = get_start_end(
        groups["snapshots"]["snapshot"],
        metadata["variables_metadata"]["snapshots_snapshot"]["units"],
    )
    data["time_series.csv"] = time_series_data(
        groups["snapshots"]["snapshot"], start_time
    )
